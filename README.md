...
What it does
Scanlines (tuned to avoid color fringing on chroma-subsampled video)
Slight color/saturation boost
Vignette (darkened corners, like tube glass curvature)
Light film grain
Subtle warm/cool color balance shift
...
its own)
Credit to Steward-fu
and bobotrax for the original
FFplay port this builds on

Installation
Download and extract this zip.
Copy the `CRTPlayer` folder into `/App/` at the root of your SD card,
so you end up with `/App/CRTPlayer/`.
Restart the device (or use Onion's app refresh if available).
You should see a new CRTPlayer icon in your Apps menu, alongside
the regular Video Player.
Videos are read from the same `Media/Videos` folder as the normal
...
`launch.sh`:
```sh
./bin/ffplay -autoexit -vf "hflip,vflip,drawgrid=w=iw:h=4:t=1:c=black@0.5,hue=s=1.3,vignette,noise=alls=9:allf=t,colorbalance=rs=0.05:gs=0.05:bs=-0.05" -i "$selected\_file" $startTimer
```
Breakdown:
Filter	What it does	Easy tweaks
`hflip,vflip`	Corrects screen orientation (device-specific, leave as-is)	—

`drawgrid=w=iw:h=4:t=1:c=black@0.5`	Scanlines	`h=` = spacing (higher = fewer lines), `@0.5` = opacity

`hue=s=1.3`	Saturation boost	`s=1.0` is neutral, higher = more color pop
...
`colorbalance=rs=0.05:gs=0.05:bs=-0.05`	Warm/cool tint	small values only, this adds up fast


Known limits of this build: the OnionOS ffplay binary is a stripped build and does not include several common ffmpeg filters — eq, rgbashift, and tmix all fail to launch (silent crash back to the file browser). A boxblur+blend bloom effect does work and launches fine, but costs enough CPU to cause audio/video sync drift — left out of the default build for that reason, but documented here in case you want to experiment: ,split[a][b];[b]boxblur=2:1[bl];[a][bl]blend=all_mode=screen:all_opacity=0.25
...
Filter chain and app packaging by Jack_Appleseed.
